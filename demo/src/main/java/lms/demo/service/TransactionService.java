...
public class TransactionService {
 ...
 // Method to view all transactions
    public static List<Transaction> viewTransactions() {
        ...
        private List<Transaction> transactions; // Assuming this holds all the transactions.

        // Method to get transactions by Member ID
        public List<Transaction> viewTransactionsByMemberId(int memberId) {
            List<Transaction> filteredTransactions = new ArrayList<>();
            
            // Loop through all transactions and filter based on Member ID
            for (Transaction transaction : transactions) {
                if (transaction.getMember().getMemberId() == memberId) {  // Assuming `getMember().getId()` fetches the Member ID
                    filteredTransactions.add(transaction);
                }
            }
            
            return filteredTransactions;  // Return the filtered list
        }
